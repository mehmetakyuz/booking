...
export function FlightStep() {
  ...
  return (
    <StepShell
      ...
      description="Flight search runs in two stages: search first, then price validation."
    >
      ...
      {flightSearch.status === 'error' ? (
        <div className="error-banner">
          ...
          <button className="button button-secondary" onClick={() => actions.ensureFlightsLoaded()} type="button">
            Retry
          </button>
        </div>
      ) : null}
        ...
        <>
          <div className="flight-list">
            {flights.map((flight) => {
              ...
              return (
                <OptionCard className="flight-card" key={flight.id} selected={selectedFlightId === flight.id} onClick={() => void handleSelectFlight(flight)}>
