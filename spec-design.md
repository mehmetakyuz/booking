...
This document is the canonical visual and interaction system for the booking engine. It is derived from `Brand guidelines V1.1.pdf` and the supplied repo assets. When inferred styling conflicts with the PDF or supplied assets, the PDF and assets win.
...
### Required CSS class inventory

Every CSS class referenced in a component JSX file **must** have a corresponding definition in the global stylesheet. A class mismatch means an unstyled element — invisible borders, broken positioning, or blank areas.

The following class families must all be defined. This list is not exhaustive but covers the areas that have historically been missed:

#### Receipt and summary panel

- `.receipt-panel` — outer wrapper with `position: relative`
- `.receipt-total`, `.receipt-total-label`, `.receipt-total-price`, `.receipt-total-old`, `.receipt-total-prices` — total row with optional struck-through old price
- `.receipt-line--sibling` — indented child lines
- `.receipt-line-text` — italic text-type line content
- `.receipt-error` — individual error item inside `.receipt-errors`
- `.summary-date`, `.summary-date-day`, `.summary-date-monthyear` — date block columns
- `.summary-nights` — nights badge in date row

#### Itinerary (preview + full modal)

- `.receipt-itinerary-preview` — section wrapper in summary sidebar
- `.itinerary-preview-row`, `.itinerary-preview-date`, `.itinerary-preview-label`, `.itinerary-preview-icons`, `.itinerary-preview-icon` — compact preview rows
- `.itinerary-event`, `.itinerary-event-date`, `.itinerary-event-label`, `.itinerary-event-components` — full itinerary day blocks
- `.itinerary-component`, `.itinerary-component-icon`, `.itinerary-component-detail`, `.itinerary-component-label`, `.itinerary-component-sublabel` — component cards inside events

#### Modal extensions

- `.modal-header`, `.modal-body` — layout for modal internals
- `.modal-bullet-list` — styled `<ul>` for included/excluded lists
- `.modal-info-list`, `.modal-info-item`, `.modal-info-label`, `.modal-info-value` — info/trip-info modal
- `.modal-itinerary` — scrollable itinerary container
- `.modal-title`, `.modal-stars`, `.modal-address`, `.modal-gallery`, `.modal-gallery-img`, `.modal-description`, `.modal-facilities` — hotel detail modal; facilities should use the same icon-led chip language as the accommodation cards rather than a plain text list

#### Rooms step

- `.rooms-section` — section spacing (same pattern as `.dates-section`)
- `.option-list` — vertical stack of option cards
- `.option-card-name`, `.option-card-subtitle`, `.option-card-stars` — card content
- `.option-card-delta`, `.option-card-total` — price display
- `.option-card .option-card-image img` — image sizing inside cards

#### General

- `.dates-section`, `.field-label`, `.field-readonly` — form section wrappers
- `.dropdown` — must have `position: relative` for `.dropdown-panel`
- `.chevron` — dropdown arrow indicator
- `.dropdown-option`, `.option-price` — dropdown list items
- `.stepper-row`, `.stepper-label`, `.stepper-controls`, `.stepper-btn`, `.stepper-value` — occupancy stepper
- `.child-ages`, `.child-age-row`, `.child-age-label`, `.child-age-select` — age selectors
- `.package-cards`, `.package-card-name`, `.package-card-desc`, `.package-card-price` — package grid
- `.chip-label`, `.chip-price` — chip sub-elements
- `.calendar`, `.calendar-nav`, `.calendar-month-label`, `.calendar-footer` — calendar wrapper and navigation
- `.calendar-day-tooltip` — positioned tooltip for flexible mode
- `.link-button` — inline link-style button
- `.btn-sm` — small button modifier
- `.loader-spinner` — spinner (in addition to `.spinner`)
