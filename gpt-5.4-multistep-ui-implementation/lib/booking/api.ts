...
export async function loadBootstrapDataServer(offerId: string, serverEndpoint: string, sessionId: string): Promise<BootstrapData> {
  ...
  const selectedBootstrapPayload: BookingPayload = {
    ...bootPayload,
    departureAirports: initialCalendar.departureAirports[0]?.iataCode
      ? [initialCalendar.departureAirports[0].iataCode]
      : undefined,
    packageGroup: initialCalendar.packageGroups[0]?.id ?? undefined,
  }
