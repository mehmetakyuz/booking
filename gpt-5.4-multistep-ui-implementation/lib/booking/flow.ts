...
export function createInitialState(bootstrap: BootstrapData): BookingState {
  ...
  return {
    ...
    flightSearch: { status: 'idle', priceChange: undefined },
...
function isTaskGroupFailedError(error: unknown) {
  return error instanceof Error && /Task group .* failed\./.test(error.message)
}

function mapFlightsLoadError(error: unknown) {
  if (isTaskGroupFailedError(error)) {
    return 'No flights are available for this stay. Choose different dates to see other options.'
  }
  return error instanceof Error ? error.message : 'Unable to load flights.'
}

function mapCarsLoadError(error: unknown) {
  if (isTaskGroupFailedError(error)) {
    return 'No car-hire options are available for this stay. Choose different dates to see other options.'
  }
  return error instanceof Error ? error.message : 'Unable to load cars.'
}
...
export function createFlow(api: FlowApi) {
  const resetToCalendarState = (payload: BookingPayload) => {
    api.patchState({
      currentStepIndex: 0,
      completedStepIds: [],
      payload,
  ...
  const refreshReceipt = async (nextPayload: BookingPayload) => {
    ...
    try {
      ...
      const currentStepIndex = api.getState().currentStepIndex
      if (receipt?.errors?.length && currentStepIndex > 0) {
        resetToCalendarState(
          buildStepOnePayload({
            ...nextPayload,
      ...
      const withPriceSeen = {
        ...nextPayload,
        priceSeen: receipt.perPersonPrice ? String(receipt.perPersonPrice) : nextPayload.priceSeen,
      }
  ...
  return {
    ...
    resetToCalendar() {
      const state = api.getState()
      resetToCalendarState(buildStepOnePayload({
        ...state.payload,
        selectedDate: undefined,
      }))
    },
      ...
      if (state.offerMeta.hasFlights && (stepAtOrBeyond('flights') || hasProductWithPrefix(reconciledPayload.products, 'F:'))) {
        ...
        try {
          ...
          patches.flightSearch = {
            status: 'error',
            error: mapFlightsLoadError(error),
          }
        }
      }
    ...
    async ensureFlightsLoaded() {
      ...
      const previousTotal = state.receipt?.totalPrice ?? 0
      api.patchState({ flightSearch: { status: 'searching', priceChange: undefined } })
      try {
        const flights = await getFlights(state.payload, (stage) => {
          api.patchState({ flightSearch: { status: stage, priceChange: undefined } })
        })
        const refreshedReceipt = state.payload.selectedDate ? await refreshReceipt(state.payload) : null
        const currentTotal = refreshedReceipt?.totalPrice ?? previousTotal
        const priceChange =
        ...
        api.patchState({
          ...
          flightSearch: flights.length
            ? { status: 'success', priceChange }
            : {
                status: 'error',
                error: 'No flights are available for this stay. Choose different dates to see other options.',
                priceChange: undefined,
              },
        })
      } catch (error) {
        api.patchState({
          flightSearch: { status: 'error', error: mapFlightsLoadError(error), priceChange: undefined },
        })
      }
    },
    ...
    async ensureCarsLoaded() {
      ...
      try {
        ...
        api.patchState({
          cars,
          carSearch: cars.length
            ? { status: 'success' }
            : { status: 'error', error: 'No car-hire options are available for this stay. Choose different dates to see other options.' },
        })
      } catch (error) {
        api.patchState({
          carSearch: { status: 'error', error: mapCarsLoadError(error) },
        })
      }
    },
    ...
    async ensureCheckoutLoaded() {
      ...
      const [checkout] = await Promise.all([
        getCheckoutMetadata(state.payload),
        state.payload.selectedDate ? refreshReceipt(state.payload) : Promise.resolve(state.receipt),
      ])
