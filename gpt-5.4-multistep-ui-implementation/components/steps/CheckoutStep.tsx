...
export function CheckoutStep() {
  ...
  const instalmentSummary = useMemo(() => {
    if (!receipt) {
      return null
    }

    if (!receipt.instalmentPayments.length) {
      return null
    }

    const selectedPlanIndex = Math.max(0, (payload.numOfInstalments ?? 1) - 1)
    const selectedSchedule = receipt.instalmentPayments[selectedPlanIndex] ?? receipt.instalmentPayments[0] ?? []
    const validPayments = selectedSchedule.filter((payment) => typeof payment.amount === 'number' && payment.amount > 0)
    const [dueNowPayment, ...futurePayments] = validPayments

    return {
      dueNowAmount: dueNowPayment?.amount ?? 0,
      dueNowDate: dueNowPayment?.payBeforeDate ?? '',
      futurePayments,
    }
  }, [payload.numOfInstalments, receipt])

  if (!checkout) {
    return (
      <StepShell eyebrow={`Step ${currentStepIndex + 1}`} title="Confirm & pay">
        ...
        {checkout.maxNrOfInstalments > 1 && receipt?.instalmentPayments.length ? (
          <section className="checkout-section">
            ...
            <div className="checkout-compact-stack">
              ...
              {instalmentSummary ? (
                <div className="checkout-instalment-summary" aria-live="polite">
                  <div className="checkout-instalment-summary-row checkout-instalment-summary-row--now">
                    <span>{instalmentSummary.dueNowDate ? `Due now · ${instalmentSummary.dueNowDate}` : 'Due now'}</span>
                    <strong>{formatPriceWithPence(instalmentSummary.dueNowAmount)}</strong>
                  </div>
                  {instalmentSummary.futurePayments.map((payment, index) => (
                    <div className="checkout-instalment-summary-row" key={`instalment-${index}-${payment.payBeforeDate ?? 'unknown'}`}>
                      <span>{payment.payBeforeDate ? `Due later · ${payment.payBeforeDate}` : 'Due later'}</span>
                      <strong>{formatPriceWithPence(payment.amount)}</strong>
                    </div>
                  ))}
                </div>
              ) : null}
