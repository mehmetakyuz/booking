export const GET_OFFER = /* GraphQL */ `
  query getOffer($offerId: ID!) {
    offer(id: $offerId) {
      ...
      informationList(
        order: [hotel_night, checkin, checkout, currency, additional_list, payment, raw_conditions]
      ) {
        id
        type
      ...
      leisures {
        ...
        units {
          ...
          images {
            id
            title
            url(w: 640, h: 350, dpr: $dpr)
          }
          image {
            id
            url(w: 197, h: 148, dpr: $dpr)
          }
        }
      }
    }
...
export const GET_DYNAMIC_PACKAGE_CHECKOUT = /* GraphQL */ `
  query getDynamicPackageCheckout(
    $offerId: ID!
    $date: Date
    $nights: Int
    $people: [PersonInput]!
