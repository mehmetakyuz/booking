...
function isAllPackagesValue(value?: string | null) {
  return value == null || value === ''
}

function packageGroupMatches(left?: string | null, right?: string | null) {
  if (isAllPackagesValue(left) && isAllPackagesValue(right)) {
    return true
  }
  return left === right
}
...
export function OccupancyStep() {
  ...
  const [packageGroup, setPackageGroup] = useState<string | null>(payload.packageGroup ?? calendar.packageGroups[0]?.id ?? null)
  ...
  const defaultPackageGroup = calendar.packageGroups[0]?.id ?? null
  const effectivePackageGroup =
    packageGroup !== undefined
      ? packageGroup
      : payload.packageGroup !== undefined
  ...
  useEffect(() => {
    ...
    setPackageGroup(payload.packageGroup ?? calendar.packageGroups[0]?.id ?? null)
  ...
  async function previewReceiptForSelection(nextDate: string, nextNights: number) {
    const nextReceipt = await actions.previewCalendarSelection({
      ...
      nights: nextNights,
      airport,
      packageGroup: effectivePackageGroup ?? '',
    })
  ...
  return (
    ...
    >
      <div className={`first-step-content${stepBusy ? ' is-busy' : ''}`}>
        <div className={`step-section first-step-filter-section${stepBusy ? ' is-busy' : ''}`}>
          ...
          <>
              ...
              <div className={`step-section first-step-filter-section${stepBusy ? ' is-busy' : ''}`}>
                ...
                <div className="package-group-list" role="list">
                  {calendar.packageGroups.map((group) => {
                    const isSelected = packageGroupMatches(effectivePackageGroup, group.id)

                    return (
                      <OptionCard
                        className="package-group-card"
                        key={group.id ?? '__all-packages__'}
                        onClick={() => {
                          if (stepBusy || isSelected) {
                            return
                          }
                          setPackageGroup(group.id ?? null)
                          setSelectedDate('')
                      ...
                      >
                        <div className="package-group-card-header">
                          <div className="package-group-card-title">
              ...
              <div className={`step-section first-step-filter-section${stepBusy ? ' is-busy' : ''}`}>
                ...
                <DropdownField
                  ...
                  options={calendar.nights.map((night) => ({
                    ...
                    label:
                      night.nights == null
                        ? `All nights · ${formatPrice(night.price)}`
            ...
            <div className="step-section">
              ...
              <div className="calendar-panel" onClick={handleCalendarWhitespaceClick}>
                ...
                {stepBusy ? (
                  <div className="calendar-loading-overlay" aria-live="polite" aria-busy="true">
                    <div className="calendar-loading-spinner" />
                  </div>
                ) : null}
              </div>
              <div className="calendar-disclaimer">
                <strong>Prices are estimated</strong>
                <p>Calculated per person based on the selected number of travellers (or at least 2 adults)</p>
                <p>Prices of included flights are subject to change during the booking process</p>
              </div>
            </div>
