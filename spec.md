...
In addition to the multi-step target, this repository also contains a sibling single-page spec that uses the same booking engine and backend contract, but expresses the journey as one progressively unlocked scrollable page.
...
- [spec-singlepage.md](./spec-singlepage.md)
  - Canonical product and UX spec for the single-page progressive-unlock booking flow
  - Defines the same booking journey as one scrollable page that unlocks later sections as valid choices are made
...
This file is the entrypoint. A build agent should read this file first, then the relevant UI spec (`spec-multistep.md` or `spec-singlepage.md`) plus the shared subsystem specs.
...
- accommodation room and meal-plan choices emit one replacement `A:` product, not separate unit and board products
