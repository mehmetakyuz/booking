...
export function normalizeReceipt(receipt: any): ReceiptData {
  return {
    ...
    instalmentPayments: (receipt?.instalmentsPayments ?? []).map((schedule: any[]) =>
      (schedule ?? []).map((payment: any) => ({
        amount: payment?.amount ?? 0,
        payBeforeDate: payment?.payBeforeDate ?? '',
      })),
    ),
...
export function normalizeAccommodations(accommodations: any[]): AccommodationOption[] {
  return (accommodations ?? []).map((item: any) => ({
    ...
    facilities: (item.facilities ?? []).map((facility: any) => ({
      name: facility.name ?? facility.nameTranslated ?? '',
    ...
    units: (item.units ?? []).map((unit: any) => ({
      ...
      facilities: (unit.facilities ?? []).map((facility: any) => ({
        name: facility.name ?? facility.nameTranslated ?? '',
        icon: facility.icon ?? null,
      })),
