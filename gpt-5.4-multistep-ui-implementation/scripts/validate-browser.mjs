...
async function chooseFixedNights(page) {
  ...
  await page.locator('.calendar-loading-overlay').waitFor({ state: 'hidden', timeout: 60000 }).catch(() => {})
  await page.waitForTimeout(1500)
}

async function chooseStay(page, preferredDayOffset = 0) {
  await chooseFixedNights(page)

  const availableDays = page.locator('.calendar-grid .calendar-day:not(.is-empty):not(.is-unavailable):not([disabled])')
  const dayCount = await availableDays.count()
  if (!dayCount) {
    throw new Error('No selectable calendar days were rendered.')
  }

  for (let index = preferredDayOffset; index < Math.min(dayCount, preferredDayOffset + 8); index += 1) {
  ...
  throw new Error(`Selecting available dates starting from offset ${preferredDayOffset} never produced a valid receipt.`)
}
...
async function validateDesktopFlow(browser) {
  ...
  await page.goto(OFFER_URL, { waitUntil: 'networkidle', timeout: 120000 })
  await page.waitForTimeout(1500)
  ...
  for (let attempt = 0; attempt < 5; attempt += 1) {
    await chooseStay(page, attempt * 3)
    ...
    if (nextLabel.includes('Flights')) {
      ...
      if (flightsVisible === 'error') {
        lastFlightError = (await page.locator('.error-banner').textContent()) ?? ''
        const recoveryButton = page.getByRole('button', { name: /Choose different dates/i })
        if (attempt === 4 || !(await recoveryButton.count())) {
          throw new Error(`Flights step returned an error instead of results: ${lastFlightError}`)
        }
  ...
  if (!reachedCheckout) {
    throw new Error(`Unable to reach checkout after retrying alternate dates. Last flight error: ${lastFlightError}`)
  }

  await page.getByRole('heading', { name: 'Lead passenger' }).waitFor({ state: 'visible', timeout: 60000 })
  await page.getByRole('heading', { name: 'Payment method' }).waitFor({ state: 'visible', timeout: 60000 })
  await page.getByRole('heading', { name: 'Terms and conditions' }).waitFor({ state: 'visible', timeout: 60000 })
